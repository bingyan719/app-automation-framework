...
from ui.core.android.constant import (DEFAULT_ADB_PATH, SDK_VERISON_NEW, IP_PATTERN)
from ui.utils.compat import decode_path
from ui.utils.snippet import get_std_encoding, reg_cleanup, split_cmd
from ui.utils.log import Logging
from ui.utils.nbsp import NonBlockingStreamReader
from ui.utils.retry import retries
from ui.core.error import (AdbError, AdbShellError, UIAutomationError, DeviceConnectionError)
...
class ADB(object):
    ...
    def __init__(self, serialno=None, host=None):
        Logging.debug('adb init begin...')
        self.serialno = serialno
        self._set_cmd_options(host)
        self.connect()
    ...
    def install_app(self, filepath, replace=False, install_options=None):
        ...
        if re.search(r"Failure \[.*?\]", out):
            print(out)
            raise UIAutomationError("Installation Failure")
    ...
    def swipe(self, tuple_x0y0, tuple_x1y1, duration=500):
        ...
        if version <= 15:
            raise UIAutomationError('swipe: API <= 15 not supported (version=%d)' % version)
        elif version <= 17:
    ...
    def get_top_activity(self):
        ...
        if m:
            ...
            raise UIAutomationError("Can not get top activity, output:%s" % dat)
    ...
    def is_screenon(self):
        ...
        raise UIAutomationError("Couldn't determine screen ON state")
    ...
    def is_locked(self):
        ...
        if not m:
            raise UIAutomationError("Couldn't determine screen lock state")
        return (m.group(1) == 'true')
    ...
    def path_app(self, package):
        ...
        if 'package:' not in output:
            raise UIAutomationError('package not found, output:[%s]' % output)
    ...
    def start_app_timing(self, package, activity):
        ...
        if not re.search(r"Status:\s*ok", out):
            raise UIAutomationError("Starting App: %s/%s Failed!" % (package, activity))
    ...
    def get_focused_package_and_activity(self):
        ...
        """
        sdk_int = int(self.sdk_version)
        if sdk_int <= 24:
            dump_cmd = "dumpsys activity activities | grep mFocusedActivity"
        else:
            dump_cmd = "dumpsys activity activities | grep mResumedActivity"
        output = self.shell(dump_cmd).strip()
        Logging.debug('focused activity info: {}'.format(output))
        out = output.split(' ')[3]
        return out
